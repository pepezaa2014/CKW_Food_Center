...
window.setTimeout(function() {
    window.location = 'sequestercard.php';
  }, 1700);
</script>
	<style >
	.btn-group .button {
...
<body>
	<div class="box2">
		<center><font size="5" color="red" face="Blern Regular">
<?php 
...
session_start();
		...
		if ($result->num_rows == 1) {
			if ($secure!=$data['Secure code']) {
				echo "รหัสยืนยันตัวตนไม่ถูกต้อง";
			}
			else {
				$sql2 = "SELECT * FROM `student` WHERE `Number` = '".$numstudent."'";
				$sentdata = "UPDATE `student` SET `Statuscard` = 'ไม่สามารถใช้งานได้', `Status free` = 'ปิด' WHERE `student`.`Number` = '$numstudent'";
...
$sMessage = "บัตรของคุณถูกอายัดเรียบร้อยแล้ว หากมีข้อสงสัยกรุณาติดต่อหน้าที่";
$chOne = curl_init();
...
curl_close( $chOne );  
$_SESSION['show4']="อายัดบัตรของคุณเรียบร้อยแล้ว";
header("location:sequestercard.php");
			}
		}
		else {
